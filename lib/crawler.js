...
async function translateToEnglish(text) {
  try {
    ...
    if (translated && translated !== text) {
      console.log(`YTS 翻译: "${text}" → "${translated}"`);
      return translated;
    }
    return text;
  } catch (e) {
    console.warn("翻译失败，使用原词:", e.message);
    return text; // fallback：直接用原词
  }
}
...
async function searchFromYTS(keyword) {
  try {
    ...
    const response = await fetchWithTimeout(
      `https://movies-api.accel.li/api/v2/list_movies.json?query_term=${encodeURIComponent(englishKeyword)}&sort_by=seeds&limit=20`,
      {
        headers: {
          "User-Agent":
